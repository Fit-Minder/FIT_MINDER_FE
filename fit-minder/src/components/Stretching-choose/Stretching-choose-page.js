...
import WristStretching from '../Stretching/WristStretching';
import EyeStretching from '../Stretching/EyeStretching';
import NeckStretching from '../Stretching/NeckStretching';
import ShoulderStretching from '../Stretching/ShoulderStretching';
...
function StretchingChoosePage() {
    ...
    const handleStartStretching = () => {
        if (selectedCheckboxes.length > 0) {
            const timerData = selectedCheckboxes.map(index => ({
                duration: timers[index],
                page: routes[index]
            }));

            const startNextStretching = (index) => {
                if (index < timerData.length) {
                    const { duration, page } = timerData[index];
                    navigate(page, { state: { timerDuration: duration, nextIndex: index + 1 } });
                } else {
                    navigate('/main');
                }
            };

            startNextStretching(0); // 첫 번째 타이머 시작
        }
    };
    ...
    return (
        <div className="stretching-choose-screen">
            ...
            <Routes>
                ...
                <Route path="/stretching/wriststretching" element={<WristStretching />} />
                <Route path="/stretching/eyestretching" element={<EyeStretching />} />
                <Route path="/stretching/neckstretching" element={<NeckStretching />} />
                <Route path="/stretching/shoulderstretching" element={<ShoulderStretching />} />
            </Routes>
        </div>
    );
}
